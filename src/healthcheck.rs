...
use crate::{BlockApi, current_time_as_secs, HealthcheckState};
// use crate::api::Api;
...
//
// pub fn graph_healthcheck(api: &dyn Api, previous_state: State<HealthcheckState>) -> Result<(), HealthcheckError> {
//     let not_indexed_blocks = previous_state.indexed_block_number.load(Ordering::Relaxed);
//     let last_checked_time = previous_state.time.load(Ordering::Relaxed);
...
pub fn get_not_indexed_block_count(api: &BlockApi) -> Result<i64, Box<dyn Error>> {
    let indexed_block = get_indexed_block_number(api)?;
    let latest_block = get_latest_block_number(api)?;
    Ok(latest_block - indexed_block)
}

pub fn get_indexed_block_number(api: &BlockApi) -> Result<i64, Box<dyn Error>> {
    let indexed_block = api.get_indexed_block_num()?;
    println!("Indexed block: {:?}", &indexed_block);
    Ok(indexed_block)
}

pub fn get_latest_block_number(api: &BlockApi) -> Result<i64, Box<dyn Error>> {
    let latest_block = (api.get_latest_block_num())?;
